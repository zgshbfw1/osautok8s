...
sed -ir "s#LISTEN_PEER_URLS=.*#LISTEN_PEER_URLS=\\\"https://${r}:2380\\\"#g"  etcd.conf
sed -ir "s#LISTEN_CLIENT_URLS=.*#LISTEN_CLIENT_URLS=\\\"https://${r}:2379,http://127.0.0.1:2379\\\"#g" etcd.conf
sed -ir "s#INITIAL_CLUSTER=.*#INITIAL_CLUSTER=INITIAL_CLUSTER=\\\"etcd1=https://${etcd1}:2380,etcd2=https://${etcd2}:2380,etcd3=https://${etcd3}:2380\\\"#g" etcd.conf
...
###################主节点创建etcd数据目录################################################################################。
ssh root@${r} "mkdir -p /var/lib/etcd/ /etc/etcd/ssl/" 
################################################################################################################################
##2.配置etcd.conf配置文件,这里要修改配置文件，需要补充。
scp -rp /k8sInstall/etcdClusterInstall/config/etcd.conf	/etc/etcd/
##3.配置etcd服务启动文件
scp -rp /k8sInstall/etcdClusterInstall/config/etcd.service 	/usr/lib/systemd/system/
##4.安装etcd服务二进制脚本
